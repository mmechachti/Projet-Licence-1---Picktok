...
####################################################################################### Ajouté par Mohamed
@_fenetre_creee
def titre_et_icone(titre: str, icone: Optional[str] = None) -> None:
    """
    Définit le titre et l'icône de la fenêtre.

    :param titre: titre de la fenêtre
    :param icone: chemin vers l'icône (.ico recommandé sous Windows,
                  .png possible sur Linux/macOS avec PIL)
    """
    assert __canevas is not None

    # Titre
    __canevas.root.title(titre)
    ...
    if icone is not None:
        ...
        # 1. Chemin tel quel (absolu ou relatif au répertoire de travail)
        possible_paths.append(icone)
        
        # 2. Relatif au répertoire du script fltk.py
        script_dir = os.path.dirname(os.path.abspath(__file__))
        possible_paths.append(os.path.join(script_dir, icone))
        
        # 3. Relatif au répertoire de l'appelant (main.py)
        try:
            import inspect
            caller_frame = inspect.stack()[1]
            caller_file = caller_frame.filename
            caller_dir = os.path.dirname(os.path.abspath(caller_file))
            possible_paths.append(os.path.join(caller_dir, icone))
        except:
            pass
        ...
        if chemin_icone is None:
            raise FileNotFoundError(f"Icône introuvable : {icone}. Recherché aux emplacements: {possible_paths}")
        
        try:
            # Windows (.ico)
            if Path(chemin_icone).suffix.lower() == ".ico":
                __canevas.root.iconbitmap(chemin_icone)
            else:
                # PNG / autres formats
                img = PhotoImage(file=chemin_icone)
                __canevas.root.iconphoto(True, img)
                # garder une référence pour éviter le garbage collection
                __canevas._window_icon = img
        except Exception as e:
            print(f"Impossible de charger l'icône : {e}", file=sys.stderr)